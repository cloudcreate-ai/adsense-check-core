...
import { callAIWithModel, getExpertModel, getExpertApiBase, getExpertApiKey, getFastModel, getFastApiBase, getFastApiKey } from './analyzer.js';
import { isProxyEnabled, proxyAnalyzeApproval } from './api-proxy.js';
import { callWithRetry } from './analyzer.js';
import { t } from '../i18n.js';
import { loadPrompt, renderPrompt } from './prompts.js';
import { extractJson, getLangName } from '../utils/prompt-utils.js';
...
export async function summarizeFinal(
  ...
  try {
    let result: any;
    if (isProxyEnabled()) {
      result = await callWithRetry(() => proxyAnalyzeApproval({
        siteUrl: report.url,
        pageSummaries: pageSummaries || 'No per-page AI analysis available.',
        siteType: report.siteType,
        siteTopic: report.siteTopic ? report.siteTopic.topic : '(unknown)',
        pagesAnalyzed: report.pages.length,
        totalDiscovered: report.samplingInfo?.totalDiscovered ?? report.pages.length,
        compositeScore: report.compositeScore,
        pageValueScore: report.pageValueScore,
        siteQuality: report.siteQuality,
        homeQuality: report.homeQuality,
        pageValueNote,
        lang,
        expert,
      }));
