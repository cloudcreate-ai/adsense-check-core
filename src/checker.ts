...
import { analyzeWithAI, analyzeBatch, type PageAiAnalysis } from './ai/analyzer.js';
import { loadCheckpoint, getPendingPages, deleteCheckpoint } from './ai/checkpoint.js';
...
function embedTypeFromSignals(sig: PageSignals): 'game' | 'video' | 'tool' | 'none' {
  // Canvas elements are typically creative tools/editors, not games
  if (sig.canvasCount > 0) return 'tool';
  // Check if iframe points to a game service
  if (sig.iframeCount > 0 && sig.iframeSrcs.some(s => isGameIframeSrc(s))) return 'game';
  if (sig.videoElementCount > 0) return 'video';
  // Generic iframe without game service — likely a tool embed
  if (sig.iframeCount > 0) return 'tool';
  return 'none';
}
...
function isGameIframeSrc(src: string): boolean {
...
function buildPageDetails(pages: Array<{ url: string; text: string; title: string; lang: string }>, aiAnalyses: PageAiAnalysis[], siteType: SiteType, lang: string, signals?: PageSignals[]): PageDetail[] {
  ...
  return pages.map(page => {
    ...
    // Warn for game/video/tool detail pages with embeds — AI can't verify embed functionality
    ...
    else if (embed === 'tool' && pageType === 'tool_detail') issues.push(t('embed.tool_verify_warning', lang));
...
export async function check(options: CheckOptions): Promise<CheckReport> {
  const { url, maxCrawl = 50, maxPages = 50, maxContent = 20, sampleMin = 20, sampleRatio = 0.2, skipAi = false, timeout = 30000, apiKey, lang = 'en', siteType: manualType, expert = false, concurrency = 5, resumeFile, onProgress } = options;
  ...
  try {
    ...
    async function launchAIBatch(batchPages: Array<{ url: string; text: string; title: string; links: string[]; lang: string; signals: PageSignals }>) {
      ...
      const p = analyzeBatch(
        batchPages.map(bp => {
          const et: 'game' | 'video' | 'tool' | 'none' = embedTypeFromSignals(bp.signals);
    ...
    // In pipeline mode, per-page analysis is already done.
    // Overall suggestions come from summarizeFinal (called later after composite score is computed).
    let pageAnalyses: PageAiAnalysis[] = aiAnalyses;
    let rateLimited = false;
    if (!skipAi) {
      try {
        // Load checkpoint for resume
        let existingPages: PageAiAnalysis[] = [];
        if (resumeFile) {
          const cp = loadCheckpoint(resumeFile);
          if (cp) {
            existingPages = cp.completedPages;
            progress(`AI: resuming from checkpoint — ${existingPages.length} pages already analyzed`);
          }
        }

        if (pageAnalyses.length === 0 && apiKeyResolved) {
          // Fallback: if pipeline didn't produce results, run analyzeWithAI
          const allPages = uniquePages.map((p, i) => {
            const sig = pages[i]?.signals ?? null;
            const et: 'game' | 'video' | 'tool' | 'none' = sig ? embedTypeFromSignals(sig) : 'none';
          ...
          // Filter out already-completed pages (resume)
          const { pending, completed } = getPendingPages(allPages, existingPages);

          if (pending.length === 0) {
            progress(`AI: all ${completed.length} pages already analyzed from checkpoint`);
            pageAnalyses = completed;
          } else {
            progress(`AI analysis: ${pending.length} remaining pages (${completed.length} from checkpoint)...`);
            ...
            rateLimited = !!(aiResult as any)._rateLimited;
          }

          // AI value analysis category (displayed in report)
          const aiItems: CheckItem[] = [];
          if ((pageAnalyses as any)._rateLimited) {
            aiItems.push({ name: t('item.ai.suggestions', lang), status: 'warn', message: `AI analysis interrupted — ${pageAnalyses.length} of ${allPages.length} pages analyzed. Use --resume to complete.`, detailList: [] });
            rateLimited = true;
          }
