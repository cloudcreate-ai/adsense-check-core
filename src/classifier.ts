...
// Patterns for tool detail pages (interactive editors, converters, calculators, etc.)
const TOOL_DETAIL_PATTERNS = [/\/tool/i, /\/editor/i, /\/studio/i, /\/converter/i, /\/calculator/i, /\/generator/i];
...
export function classifyPage(url: string): PageType {
  ...
  // Tool detail pages (interactive editors, converters, etc.)
  // Only for paths NOT under content prefixes — e.g. /tool, /en/tool
  if (TOOL_DETAIL_PATTERNS.some(p => p.test(pathname))) {
    const hasContentPrefix = CONTENT_PREFIXES.some(prefix => {
      const direct = pathname.includes(prefix);
      const langPrefix = pathname.match(/^\/[a-z]{2}\//);
      const withLang = langPrefix ? pathname.slice(3).includes(prefix) : false;
      return direct || withLang;
    });
    if (!hasContentPrefix) return 'tool_detail';
  }
