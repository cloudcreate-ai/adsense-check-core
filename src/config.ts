...
export interface AdsenseCheckApiConfig {
  enabled: boolean;
  apiBase?: string;
  apiKey?: string;
}
...
export interface AdsenseConfig {
  ...
  // API proxy (route AI requests through adsense-check-api server)
  adsenseCheckApi: AdsenseCheckApiConfig;
...
export const DEFAULTS: AdsenseConfig = {
  ...
  adsenseCheckApi: {
    enabled: true,
    apiBase: 'https://adsense-check-api.cloudcreate.ai',
    apiKey: 'adsense-check-api-key-free',
  },
};
...
function mergeConfig(base: AdsenseConfig, override: Partial<AdsenseConfig>): AdsenseConfig {
  ...
  if (override.adsenseCheckApi != null) result.adsenseCheckApi = { ...result.adsenseCheckApi, ...override.adsenseCheckApi };
...
export function saveConfig(config: Partial<AdsenseConfig>, path: string): void {
  ...
  const lines = [
    ...
    '',
    '# adsense-check-api (route AI requests through proxy server, no local AI key needed)',
    'adsenseCheckApi:',
    `  enabled: ${config.adsenseCheckApi?.enabled !== undefined ? config.adsenseCheckApi.enabled : true}`,
    `  apiBase: "${config.adsenseCheckApi?.apiBase ?? 'https://adsense-check-api.cloudcreate.ai'}"`,
    `  apiKey: "${config.adsenseCheckApi?.apiKey ?? 'adsense-check-api-key-free'}"`,
    '',
  ];
